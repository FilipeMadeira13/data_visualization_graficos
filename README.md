# Data Visualization com Python

## Sobre o Projeto

Este projeto foi desenvolvido no **Google Colab** com o objetivo de aplicar conceitos fundamentais de **Análise Exploratória de Dados (EDA)** e boas práticas de **Data Visualization** utilizando Python.

A proposta central é explorar distribuições, relações entre variáveis e padrões estatísticos por meio de visualizações bem estruturadas e interpretáveis.

Mais do que apenas gerar gráficos, o projeto demonstra:

* Capacidade analítica
* Interpretação estatística
* Comunicação visual clara
* Organização de código em ambiente notebook

---

## Tecnologias Utilizadas

* Python 3
* Pandas
* NumPy
* Matplotlib
* Seaborn
* Google Colab

---

## Visualizações Desenvolvidas

O notebook inclui a construção e análise de:

* Histogramas (colunas e linhas)
* Boxplots
* Violin Plots
* Gráficos de Dispersão
* Inserção de média, mediana e moda nos gráficos
* Customizações de layout, tema e anotações
* Tratamento e rejeição de outliers por percentil

Cada gráfico foi construído com foco em:

* Clareza interpretativa
* Legibilidade
* Coerência estatística
* Boas práticas de visualização

---

## Conceitos Aplicados

Durante o desenvolvimento foram trabalhados:

* Distribuição de variáveis numéricas
* Assimetria e dispersão
* Identificação visual de outliers
* Comparação entre amostras
* Correlação visual entre variáveis
* Aplicação prática de percentis
* Estilização avançada com Matplotlib

---

## Como Executar o Projeto

### 🔹 Opção 1 – Abrir no Google Colab (Recomendado)

1. Faça upload do arquivo `.ipynb` no Google Colab
   ou
2. Abra diretamente pelo GitHub usando a opção **"Open in Colab"**

---

### 🔹 Opção 2 – Executar Localmente

1. Clone o repositório:

```bash
git clone https://github.com/FilipeMadeira13/data_visualization_graficos.git
```

2. Instale as dependências:

```bash
pip install pandas numpy matplotlib seaborn notebook
```

3. Execute:

```bash
jupyter notebook
```

---

## Estrutura do Projeto

```
data_visualization_graficos
 ┣ Projeto Inicial - Dataviz - Gráficos.ipynb
 ┗ README.md
```

> Projeto desenvolvido integralmente em notebook, com foco em exploração e visualização.

---

## Diferenciais do Projeto

* Código organizado e comentado
* Aplicação prática de conceitos estatísticos
* Visualizações com propósito analítico (não apenas estético)
* Uso consciente de medidas centrais e dispersão
* Tratamento de valores extremos

---

## Próximos Passos

* Aplicar a mesma estrutura em dataset real (Kaggle ou dados públicos)
* Modularizar funções em scripts `.py`
* Criar dashboard interativo (Streamlit ou Plotly)
* Implementar storytelling orientado a problema de negócio
* Evoluir para projeto com pipeline completo de EDA

---

## Objetivo Profissional

Este projeto faz parte da minha preparação para atuar como **Analista de Dados**, com foco em:

* Análise exploratória
* Visualização estratégica
* Comunicação de insights
* Preparação para entrevistas técnicas

---

## Autor

**Filipe Madeira**
- GitHub: [https://github.com/FilipeMadeira13](https://github.com/FilipeMadeira13)
- LinkedIn: [https://www.linkedin.com/in/carlos-filipe-madeira-de-souza-16211922a](https://www.linkedin.com/in/carlos-filipe-madeira-de-souza-16211922a)

